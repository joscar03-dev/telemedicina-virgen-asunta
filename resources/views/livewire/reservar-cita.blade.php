<div class="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-md">
    <div class="flex items-center justify-between">
        ...
        <div class="flex-1">
            <img src="{{ asset('images/3.jpg') }}" class="w-full h-auto object-cover rounded-md">
        </div>
        ...
        @if (!$showPacienteForm && $paciente_id)
            <form wire:submit.prevent="saveCita" class="mt-6 bg-gray-50 p-6 rounded-md shadow-md">
                ...
                <div class="mb-4">
                    <label for="voucher" class="block text-sm font-medium text-gray-700">Subir Voucher</label>
                    <input type="file" id="voucher" wire:model="voucher"
                        class="mt-2 p-3 border border-gray-300 rounded w-full focus:ring-2 focus:ring-blue-500">
                    @error('voucher')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
